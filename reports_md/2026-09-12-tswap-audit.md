...
TSwap is a decentralized, permissionless automated market maker (AMM) protocol modeled after Uniswap V2. It allows any ERC20 token to be paired against WETH in its own isolated liquidity pool, enabling users to:

- **Provide liquidity** to a pool in exchange for LP (liquidity provider) tokens representing their proportional share of that pool's reserves.
- **Swap** between WETH and the paired ERC20 token, with pricing determined by the constant product formula `(x * y = k)` and a 0.3% swap fee that accrues to liquidity providers.
- **Withdraw** their share of the underlying reserves at any time by burning their LP tokens.

The protocol is composed of two core contracts:

- `PoolFactory.sol` — a factory contract that deploys and tracks a single `TSwapPool` for each ERC20 token users wish to trade against WETH. Prevents duplicate pools for the same token.
- `TSwapPool.sol` — an ERC20 contract (the LP token itself) that holds the reserves for one token/WETH pair and implements the `deposit`, `withdraw`, `swapExactInput`, and `swapExactOutput` logic.

# Disclaimer

@mdaimanw3 makes all effort to find as many vulnerabilities in the code in the given time period, but holds no responsibilities for the findings provided in this document. A security audit by the team is not an endorsement of the underlying business or product. The audit was time-boxed and the review of the code was solely on the security aspects of the Solidity implementation of the contracts.
...
# Audit Details 
Reviewed Commit Hash: `08911d593f8ccffb3f65b523d5205cc4865d8bac`
...
## Roles
- **Liquidity Provider** — Deposits WETH and a proportional amount of the paired ERC20 token into a pool via `deposit()`, receiving LP tokens representing their share of the pool. Can withdraw their proportional share of both underlying tokens at any time via `withdraw()`. Earns a share of the 0.3% swap fee generated by trading activity.
- **Swapper / Trader** — Exchanges one token for the other via `swapExactInput`, `swapExactOutput`, or the `sellPoolTokens` convenience wrapper, paying a 0.3% fee to the pool on each trade.
- **Pool Deployer** — Deploys the `PoolFactory` once, fixing which token address is treated as WETH for the system. After that, pool creation is permissionless: any account may call `PoolFactory::createPool()` to launch a new pool for a token of their choosing. There is no privileged owner/admin role in either contract beyond this initial factory deployment.
